...
export const getStatusInfo = (status) => {
  const statusMap = {
    // Requester statuses
    'in_progress': { text: 'In Progress', color: '#2196f3', bgColor: '#e3f2fd', icon: 'refresh' },
    'pending_review': { text: 'Pending Review', color: '#ff9800', bgColor: '#fff3e0', icon: 'time' },
    'completed': { text: 'Completed', color: '#4caf50', bgColor: '#e8f5e8', icon: 'checkmark' },
    'awaiting_expert': { text: 'Finding Expert', color: '#9c27b0', bgColor: '#f3e5f5', icon: 'search' },
    'disputed': { text: 'Disputed', color: '#f44336', bgColor: '#ffebee', icon: 'warning' },
    'cancelled': { text: 'Cancelled', color: '#757575', bgColor: '#f5f5f5', icon: 'close' },
    
    // Expert statuses
    'working': { text: 'Working', color: '#2196f3', bgColor: '#e3f2fd', icon: 'hammer' },
    'delivered': { text: 'Delivered', color: '#ff9800', bgColor: '#fff3e0', icon: 'send' },
    'payment_received': { text: 'Payment Received', color: '#4caf50', bgColor: '#e8f5e8', icon: 'wallet' },
    'revision_requested': { text: 'Revision Requested', color: '#ff5722', bgColor: '#fbe9e7', icon: 'refresh' },
  };
  
  return statusMap[status] || { text: status, color: '#757575', bgColor: '#f5f5f5', icon: 'help' };
};

// Get urgency styling
export const getUrgencyStyle = (urgency) => {
  const styles = {
    'high': { backgroundColor: '#ffebee', color: '#f44336', icon: 'flame' },
    'medium': { backgroundColor: '#fff3e0', color: '#ff9800', icon: 'flash' },
    'low': { backgroundColor: '#e8f5e8', color: '#4caf50', icon: 'leaf' }
  };
  
  return styles[urgency] || { backgroundColor: '#f5f5f5', color: '#757575', icon: 'list' };
};
...
export const getFileIcon = (type) => {
  ...
  return icons[type?.toLowerCase()] || 'attach';
};
